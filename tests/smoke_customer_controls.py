...
Covers the changes in the phase2-customer-controls branch:
  ...
  T1 Configurable branding: APP_NAME, COMPANY_NAME, SUPPORT_EMAIL,
     ...
     pages and footer when set via env, and defaults to "Cutover" /
...
def t1b_branding_defaults():
    ...
    base = c.get("/login").get_data(as_text=True)
    assert "Cutover" in base, "default COMPANY_NAME should be Cutover"
    print("T1b OK: default branding shows Cutover and placeholder support/security emails")
