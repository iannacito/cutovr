...
def plan_recovery_reclass_groups(
    grouped_by_txn: "OrderedDict[str, list[dict]]",
    diag_sink: "list | None" = None,
) -> list[dict]:
    """Detect and group reversal/reclass legs that TotRec detector misses.

    Phase A: Recovery Reclass Detector
    ==================================

    For each month found in the data, searches for reversal/reclass patterns:

    1. Anchor: CER row with description == "Total of Recoveries" (debit D)
    2. Candidate reclass legs: single-line entries with credit to non-bank account AND any of:
       * vendor_name/description/memo contains: "Expense Recovery", "Recovery", "Refund", "Reclass"
       * mirrors a same-account expense debit elsewhere (opposite side; exact OR within $0.01)
       * shares CER/recovery source-journal token with anchor

    Reconciliation: legs_sum vs anchor D within $0.01 tolerance
    * Exact (Δ=0) or within_tol (|Δ|≤0.01) → confident group, emit balanced group
    * Not reconciled → flag diagnostic (never force-post), do NOT group

    Confidence tiers:
    * exact (Δ=0) → auto-group
    * within_tol (|Δ|≤0.01) → auto-group
    * partial (some matched, residual) → flag for review, do NOT auto-group
    * unmatched (no anchor or no cluster) → flag for review, do NOT auto-group

    Returns a list of validated groups ready to merge. Each group has:
        {
            "group_id": "YYYY-MMRCLS",
            "token": "YYYY-MMRCLS",
            "transaction_ids": [...],
            "rows": [...],
            "debits": Decimal,
            "credits": Decimal,
            "balanced": True,
            "reclass_anchor_debit": Decimal,
            "month_key": "YYYY-MM",
            "confidence": "exact" | "within_tol" | "partial" | "unmatched",
        }

    Test fixtures (March confirmed entries):
    * 260558/260559: Legal -38.50 / +38.50 (mirror pattern)
    * 261995/261996: Legal -150 / +150 (mirror pattern)
    * 263360/263361: Legal -121 / +121 (mirror pattern)
    * 260935/260936: BankChg -5.65 / +5.65 (mirror pattern)
    * 260654: Legal -80.83 (marked "Expense Recovery")
    * 265892: Legal "WF-5722" source, +X counterpart
    * Id-less "Total of Recoveries" CER row: debit 1,544.85

    Non-reconciling case (deliberate fixture):
    * If one reversal leg is dropped, Δ will not close → flag instead of post

    Groups that don't balance exactly within tolerance are skipped (no silent
    corrections) — the caller should flag them for manual review instead of forcing.
    ...
    # Process each month for Recovery Reclass grouping.
    for month_key, month_rows in rows_by_month.items():
        # Find the Total of Recoveries adjustment row (CER, description == "Total of Recoveries").
        reclass_anchor_row = None
        for r in month_rows:
            journal = source_journal_token(r) or ""
            if (
                journal == "CER"
                ...
                reclass_anchor_row = r
                break

        if not reclass_anchor_row:
            result = "no_total_of_recoveries_row"
            if diag_sink is not None:
                diag_sink.append({
                    "month_key": month_key,
                    "detector": "recovery_reclass",
                    "found_anchor_row": False,
                    "reclass_leg_count": 0,
                    "reclass_anchor_debit": None,
                    "reclass_legs_credits": None,
                    "expected_credits": None,
                    "balance_delta": None,
                    "result": result,
                    "token": None,
        ...
        # Extract the Total of Recoveries debit from the anchor.
        reclass_anchor_debit = _row_money(reclass_anchor_row, "debit")
        if reclass_anchor_debit <= 0:
            result = "nonpositive_debit"
            if diag_sink is not None:
                diag_sink.append({
                    "month_key": month_key,
                    ...
                    "reclass_leg_count": 0,
                    "reclass_anchor_debit": f"{reclass_anchor_debit:.2f}" if reclass_anchor_debit else None,
                    "reclass_legs_credits": None,
                    "expected_credits": None,
                    "balance_delta": None,
                    "result": result,
        ...
        # Find candidate reclass legs: single-line entries with credit to non-bank account
        # matching keyword patterns, mirror patterns, or source-journal tokens.
        reclass_legs = []
        for r in month_rows:
            # Skip the anchor itself
            if id(r) == id(reclass_anchor_row):
                continue

            credit = _row_money(r, "credit")
            debit = _row_money(r, "debit")

            # Only consider rows with a credit side (reclass typically credits the recovery source)
            if credit <= 0:
                continue

            # Check for keyword matches in vendor_name, description, memo
            keywords = ("Expense Recovery", "Recovery", "Refund", "Reclass")
            has_keyword = False
            for key in ("vendor_name", "description", "memo"):
                text = (r.get(key) or "").strip().lower()
            ...
            if account_number or account_name:
                # Look for a matching debit on the same account
                for other_r in month_rows:
                    if id(other_r) == id(r) or id(other_r) == id(reclass_anchor_row):
                        continue
                    if (other_r.get("account_number") == account_number or
                        other_r.get("account_name") == account_name):
                        other_debit = _row_money(other_r, "debit")
                        # Check if amounts match exactly or within $0.01 tolerance
                        if other_debit > 0 and round(abs(other_debit - credit), 2) <= 0.01:
                            reclass_legs.append(r)
                            break
        ...
        if not reclass_legs:
            ...
            if diag_sink is not None:
                diag_sink.append({
                    ...
                    "found_anchor_row": True,
                    "reclass_leg_count": 0,
                    "reclass_anchor_debit": f"{reclass_anchor_debit:.2f}",
                    "reclass_legs_credits": None,
                    "expected_credits": None,
                    "balance_delta": None,
                    "result": result,
        ...
        # Compute totals for the group.
        group_rows = [reclass_anchor_row] + reclass_legs
        debits, credits = _txn_totals(group_rows)

        # Verify the group balances: sum(reclass_leg credits) == reclass_anchor_debit
        reclass_legs_credits = sum(_row_money(r, "credit") for r in reclass_legs)
        expected_credits = reclass_anchor_debit

        # Reconciliation tolerance: $0.01
        balance_delta = reclass_legs_credits - expected_credits
        delta_amount = round(abs(balance_delta), 2)

        # Determine confidence tier
        if delta_amount == 0:
            confidence = "exact"
        elif delta_amount <= 0.01:
            confidence = "within_tol"
        else:
            confidence = "partial"

        # Only group if exact or within_tol
        if confidence not in ("exact", "within_tol"):
            result = "unbalanced"
            if diag_sink is not None:
                diag_sink.append({
                    "month_key": month_key,
                    "detector": "recovery_reclass",
                    "found_anchor_row": True,
                    "reclass_leg_count": len(reclass_legs),
                    "reclass_anchor_debit": f"{reclass_anchor_debit:.2f}",
                    "reclass_legs_credits": f"{reclass_legs_credits:.2f}",
                    "expected_credits": f"{expected_credits:.2f}",
                    "balance_delta": f"{balance_delta:.2f}",
                    "result": result,
                    "token": None,
        ...
        # Belt-and-suspenders: the rows we actually post as one JE must balance
        # to the cent, independent of the recovery reconciliation formula above.
        if round(debits, 2) != round(credits, 2):
            result = "je_unbalanced"
            if diag_sink is not None:
                diag_sink.append({
        ...
        # The group balances exactly within tolerance. Collect unique transaction IDs from all rows.
        transaction_ids: set[str] = set()
        for r in group_rows:
            tid = (r.get("transaction_id") or r.get("reference_number") or "").strip()
            if tid:
        ...
        groups.append({
            ...
            "credits": credits,
            "balanced": debits == credits,
            "reclass_anchor_debit": reclass_anchor_debit,
            "month_key": month_key,
            "confidence": confidence,
        })
