...
import post_push_reconciliation as ppr
...
def _gl_date_range(rows: list[dict]) -> tuple[str, str]:
    """(start_date, end_date) from GL rows, expanded to full month boundaries.

    Raises ValueError when no parseable dates exist."""
    ...
    for r in rows:
        ...
        for fmt in ("%b %d/%y", "%d/%m/%Y", "%m/%d/%Y"):
            try:
                dates.append(_dt.datetime.strptime(s, fmt).date())
                break
            except ValueError:
                continue
    if not dates:
        raise ValueError("No parseable dates in GL rows — cannot determine QBO pull range.")
    mn, mx = min(dates), max(dates)
    start = mn.replace(day=1).isoformat()
    end = mx.replace(day=_cal.monthrange(mx.year, mx.month)[1]).isoformat()
    return start, end


def _posted_doc_numbers(job: dict) -> set:
    """DocNumbers of the JEs THIS job created, as QBO echoed them back.

    job["qbo_results"] holds the JournalEntry objects returned by QBO's
    create responses, so their DocNumber is the stored (already truncated —
    QBO caps DocNumber at 21 chars) value. NEVER rebuild these from internal
    group ids: GROUP doc numbers can exceed 21 chars pre-truncation and
    would then fail to match, wrongly excluding grouped batches as
    out-of-scope."""
    docs = set()
    for r in (job.get("qbo_results") or []):
...
def _run_gl_recon(job: dict, qbo) -> dict:
    """Re-parse encrypted GL source, fetch QBO GL report, reconcile.

    Independent of any parsed-row snapshot on the job — re-derives from the
    encrypted file (with SHA-256 integrity guard) so the recon verifies the
    source document, not the import's memory of it. QBO rows are filtered to
    the DocNumbers this job posted, so other jobs / the OPEN-BAL JE in the
    same date range don't flag as false qbo-only variances.
    Stores result in job["gl_recon"]; sets job["recon_status"]="verified"
    on overall_pass. Raises ValueError for file/integrity problems; callers
    handle QBOError."""
    enc_filename = job.get("encrypted_file")
    if not enc_filename:
        raise ValueError("Source file not available — job has no encrypted_file reference.")
    enc_path = UPLOAD_DIR / enc_filename
    if not enc_path.exists():
        raise ValueError(
            f"Source file '{enc_filename}' has been purged from disk. "
            "Re-upload the original GL export to run post-push verification."
        )
    tmp_path = UPLOAD_DIR / f"_recon_tmp_{job['id']}.csv"
    try:
        decrypt_file(enc_path, tmp_path)
        stored_sha = job.get("file_sha256") or ""
        if stored_sha:
            current_sha = sha256_of_file(tmp_path)
            if current_sha != stored_sha:
                raise ValueError(
                    "Source file integrity check failed: SHA-256 mismatch. "
                    "The encrypted file may be corrupted — re-upload to restore."
                )
        pclaw_rows = load_general_ledger_csv(tmp_path)
    finally:
        try:
            ...
            pass
    if not pclaw_rows:
        raise ValueError("Re-parse produced no GL rows. File may be empty or malformed.")
    start_date, end_date = _gl_date_range(pclaw_rows)
    raw_qbo_rows = qbo.get_general_ledger(start_date, end_date)
    posted_docs = _posted_doc_numbers(job)
    raw_count = len(raw_qbo_rows)
    if posted_docs:
        qbo_rows = [
            r for r in raw_qbo_rows
            if (r.get("doc_number") or "").strip() in posted_docs
        ]
        out_of_scope_count = raw_count - len(qbo_rows)
        doc_filter_applied = True
    else:
        qbo_rows = raw_qbo_rows
        out_of_scope_count = None
        doc_filter_applied = False
    report = ppr.build_post_push_reconciliation(pclaw_rows, qbo_rows)
    report["_meta"] = {
        "run_at": datetime.utcnow().isoformat() + "Z",
        "start_date": start_date,
        "end_date": end_date,
        "pclaw_row_count": len(pclaw_rows),
        "qbo_row_count": len(qbo_rows),
        "qbo_rows_out_of_scope": out_of_scope_count,
        "doc_filter_applied": doc_filter_applied,
        "note": (
            "Activity-only comparison: opening balances are posted and "
            "verified by the Opening Balance step, not this recon."
        ),
    }
    job["gl_recon"] = report
    if report.get("overall_pass"):
        job["recon_status"] = "verified"
    return report


@app.route("/jobs/<job_id>/verify-gl-recon", methods=["GET", "POST"])
@login_required
def verify_gl_recon(job_id: str):
    """GET: show GL reconciliation state. POST: trigger reconciliation run.

    Hard gate: verified only when overall_pass or every flagged account has
    a typed disposition (stored on the job + audit_logs)."""
    user = current_user()
    if not user:
        return redirect(url_for("login"))
    job = jobs.get(job_id) or db.hydrate_job(job_id)
    if not job or job.get("firm_id") != user["firm_id"]:
        flash("Job not found.", "error")
        return redirect(url_for("migration_nexus"))
    if job.get("report_type") != REPORT_GENERAL_LEDGER:
        flash("Post-push verification is only available for GL imports.", "error")
        return redirect(url_for("job_detail", job_id=job_id))
    if not job.get("import_summary"):
        flash("This job has not been imported to QuickBooks yet.", "error")
        return redirect(url_for("job_detail", job_id=job_id))
    if request.method == "POST":
        qbo, _qbo_conn = _get_qbo_client(job_id, user)
        if qbo is None:
            flash(
                "QuickBooks connection is required to run verification. "
                "Reconnect QBO and try again.",
                "error",
            )
            return redirect(url_for("verify_gl_recon", job_id=job_id))
        try:
            _run_gl_recon(job, qbo)
        except ValueError as ve:
            job["gl_recon"] = {
                "_error": str(ve),
                "_meta": {"run_at": datetime.utcnow().isoformat() + "Z"},
            }
        except Exception as e:  # noqa: BLE001
            job["gl_recon"] = {
                "_error": f"QBO API error: {e}",
                "_meta": {"run_at": datetime.utcnow().isoformat() + "Z"},
            }
        _save_job(job_id)
        _audit("gl_recon_run", target_type="job", target_id=job_id,
                details=f"overall_pass={job.get('gl_recon', {}).get('overall_pass')}")
        return redirect(url_for("verify_gl_recon", job_id=job_id))
    recon = job.get("gl_recon") or {}
    dispositions = job.get("gl_dispositions") or {}
    flagged = [
        d for d in (recon.get("detail") or [])
        if d.get("status") in ("diff", "pclaw-only", "qbo-only")
    ]
    flagged_keys = {d["account_number"] for d in flagged}
    pending_keys = flagged_keys - set(dispositions.keys())
    if recon and not recon.get("_error"):
        if recon.get("overall_pass") or not pending_keys:
            if job.get("recon_status") != "verified":
                job["recon_status"] = "verified"
                _save_job(job_id)
    return render_template(
        "verify-gl-recon.html",
        job=job,
        job_id=job_id,
        recon=recon,
        ...
        can_verify=(
            bool(recon) and not recon.get("_error") and not pending_keys
        ),
    )


@app.route("/jobs/<job_id>/gl-disposition", methods=["POST"])
@login_required
def submit_gl_disposition(job_id: str):
    """Save a typed operator disposition for one flagged GL account.

    Stored in job["gl_dispositions"] and audit_logs. Advances recon_status
    to "verified" when all flagged accounts are dispositioned."""
    user = current_user()
    if not user:
        return redirect(url_for("login"))
    job = jobs.get(job_id) or db.hydrate_job(job_id)
    if not job or job.get("firm_id") != user["firm_id"]:
        flash("Job not found.", "error")
        return redirect(url_for("migration_nexus"))
    acct_num = (request.form.get("account_number") or "").strip()
    acct_name = (request.form.get("account_name") or "").strip()
    reason = (request.form.get("reason") or "").strip()
    if not acct_num or not reason:
        flash("Account number and reason are required.", "error")
        return redirect(url_for("verify_gl_recon", job_id=job_id))
    recon_run_at = ((job.get("gl_recon") or {}).get("_meta") or {}).get("run_at", "")
    if "gl_dispositions" not in job:
        job["gl_dispositions"] = {}
    job["gl_dispositions"][acct_num] = {
        "account_name": acct_name,
        "reason": reason,
        "dispositioned_at": datetime.utcnow().isoformat() + "Z",
        "dispositioned_by": user.get("email") or str(user.get("id")),
        "recon_run_at": recon_run_at,
    }
    _audit(
        "gl_recon_disposition",
        target_type="job",
        target_id=job_id,
        details=f"acct={acct_num} ({acct_name}): {reason[:200]}",
    )
    recon = job.get("gl_recon") or {}
    flagged_keys = {
        d["account_number"]
        for d in (recon.get("detail") or [])
        if d.get("status") in ("diff", "pclaw-only", "qbo-only")
    }
    if flagged_keys and flagged_keys.issubset(set(job["gl_dispositions"].keys())):
        job["recon_status"] = "verified"
    _save_job(job_id)
    flash(f"Disposition saved for account {acct_num}.", "success")
    return redirect(url_for("verify_gl_recon", job_id=job_id))
    ...
    try:
        ...
        if is_gl_format(fieldnames):
            ...
            job["import_summary"] = {
                ...
                "source_row_count": (job.get("summary") or {}).get("row_count", 0),
            }
            _record_checkpoint(job, job_id, job_checkpoints.COMPLETED)
            # Flag for post-push reconciliation. Clears to "verified" once
            # the operator runs the GL recon and all accounts pass or are
            # dispositioned.
            job["recon_status"] = "not_verified"
...
def preview_import(job_id):
    ...
    if rows is not None and preview_error is None:
        try:
            ...
            logging.getLogger("app").exception(
                "preview_import: build_dry_run_preview failed for job %s", job_id
            )
            preview_error = f"Could not build the import preview: {exc}"
        ...
        if preview is not None:
            ...
            try:
                ...
                logging.getLogger("app").warning(
                    "preview_import: auto_balance(case1/2) failed for job %s: %s", job_id, exc
                )
